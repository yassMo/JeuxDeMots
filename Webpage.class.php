<?php
	class Webpage{
		...
		public function searchLinks () {
			...
			for( $i = 0 ; $i < $size ; $i++ ) {
    			if ( 0 == stripos($linkslist[1][$i],"/wiki/",0) ) {
					$linkslist[1][$i] = "http://fr.wikipedia.org".$linkslist[1][$i];
				}
    			if ( 0 == stripos($linkslist[1][$i],"//",0) ) {
					$linkslist[1][$i] = "http:".$linkslist[1][$i];
				}
				if ( 1 == preg_match( "/^(.*)\.[jpg|png|jpeg|gif]$/i", $linkslist[1][$i] ) ) {
					$linkslist[1][$i] = "";
				}
				if ( 0 == preg_match( "/^http:\/\/fr\.wikipedia\.org.*/i", $linkslist[1][$i] ) ) {
					$linkslist[1][$i] = "";
				}
			}
		...
		public function isLinked ( $webpage ) {
			if ( strcmp ($webpage->getContents(),"") == 0 ) return false;
			$size = sizeof($webpage->getLinks());
			for ( $i = 0 ; $i < $size ; $i++) {
				if ( 0 == strcmp($this->address,$webpage->getLink($i)) ) {
					$webpage->incrWeight();
					return true;
				}	
			}
